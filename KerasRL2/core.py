...
from tensorflow.keras.layers import Lambda, Input, Layer, Dense, Flatten, InputLayer
...
class AbstractDQNAgent(Agent):
    """Write me
    """
    def __init__(self, nb_actions, memory, gamma=.99, batch_size=1, nb_steps_warmup=1000,
...
class DQNAgent(AbstractDQNAgent):
    ...
    def backward(self, reward, terminal):
        ...
        if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
            ...
            state0_batch = self.process_state_batch(state0_batch).astype('float32')
            state1_batch = self.process_state_batch(state1_batch).astype('float32')
...
model.build(input_shape=(states,))
